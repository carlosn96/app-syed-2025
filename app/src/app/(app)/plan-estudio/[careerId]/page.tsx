        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (planData.length === 0) {
                  setError(null); // Clear error if career exists but has no plan
                }
                ...
                if (err instanceof Error && err.message.includes("Unexpected end of JSON input")) {
                    ...
                    // Let's get the career info to confirm.
                    try {
                        const careersData = await getCareers();
                        const currentCareer = careersData.find(c => c.id === careerId) || null;
                        setCareerInfo(currentCareer);
                        if (currentCareer) {
                            setStudyPlans([]); // It's an empty plan, not an error
                            setError(null);
                        } else {
                            setError("No se encontró la carrera o no tiene un plan de estudios asignado.");
                        }
                    } catch (finalErr: any) {
                         setError(finalErr.message || "Error al cargar los datos del plan de estudio.");
                    }
                } else {
